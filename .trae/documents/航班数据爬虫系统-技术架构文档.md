...
| /input_handler | 输入文件处理模块，读取Excel中的航班信息 |
| /web_automation | 网页自动化模块，控制浏览器操作 |
| /ocr_processor | OCR识别模块，处理验证码和时间图片 |
| /data_extractor | 数据提取模块，解析网页内容 |
| /output_handler | 输出处理模块，生成Excel结果文件 |
| /config_manager | 配置管理模块，处理系统参数 |
...
- **文件命名规则**：flight_crawler_YYYY-MM-DD.log（如flight_crawler_2025-01-15.log）
